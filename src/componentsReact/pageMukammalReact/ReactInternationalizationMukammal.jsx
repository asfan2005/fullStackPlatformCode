...
import dataInternationalizationMukammal from '../dataMukammalReact/dataInternationalizationMukammal'
import { Prism as SyntaxHighlighter } from 'react-syntax-highlighter'
import { vscDarkPlus } from 'react-syntax-highlighter/dist/esm/styles/prism'
import { toast } from 'react-hot-toast'

function ReactInternationalizationMukammal() {
        ...
        <div className="space-y-2">
          ...
          <div>
            <ul className="list-disc pl-5 mt-1">
              {parsedResult.methods ? 
                parsedResult.methods.map((method, index) => (
                  <li key={index}>{method}</li>
                )) :
              parsedResult.functionality ? 
                parsedResult.functionality.map((func, index) => (
                  <li key={index}>{func}</li>
                )) :
              parsedResult.types ? 
                parsedResult.types.map((type, index) => (
                  <li key={index}>{type}</li>
                )) :
              parsedResult.implementations ? 
                parsedResult.implementations.map((impl, index) => (
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-center mb-8 text-indigo-600">React Internationalization</h1>
      
      <div className="flex flex-col gap-8">
        {dataInternationalizationMukammal.map((item) => (
...
export default ReactInternationalizationMukammal