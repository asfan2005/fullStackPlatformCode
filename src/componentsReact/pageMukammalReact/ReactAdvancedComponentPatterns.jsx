...
import { reactAdvancedComponentPatternsData } from '../dataMukammalReact/dataAdvancedComponentPatterns';
...
const CodeBlock = memo(({ code }) => {
  ...
  const handleCopy = async () => {
    try {
      ...
      // 2 sekunddan keyin yana "Copy" ga qaytarish
      setTimeout(() => {
        setCopyStatus('Copy');
      }, 2000);
...
function ReactAdvancedComponentPatterns() {
  return (
      ...
      <div className="container mx-auto px-4 py-16 max-w-7xl">
        ...
        <div className="text-center mb-16">
          <h1 className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 hover:from-purple-600 hover:to-blue-600 transition-all duration-300 md:text-6xl mb-8">
            {reactAdvancedComponentPatternsData.title}
          </h1>
        </div>

        {/* Tutorials Section */}
        <div className="space-y-16">
          {reactAdvancedComponentPatternsData.tutorials.map((tutorial) => (
...
export default memo(ReactAdvancedComponentPatterns);