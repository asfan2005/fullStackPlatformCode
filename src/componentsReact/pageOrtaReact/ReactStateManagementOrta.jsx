...
import dataStateManagementOrta from '../dataOrtaReact/dataStateManagmentOrta';
...
function ReactStateManagementOrta() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      <div className="container mx-auto px-4 py-16 max-w-7xl">
        {dataStateManagementOrta.map((section) => (
          <div key={section.id} className="mb-20">
            {/* Header Section */}
            <div className="text-center space-y-8 mb-12">
              <h1 className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 hover:from-purple-600 hover:to-blue-600 transition-all duration-300">
                {section.title}
              </h1>
              ...
              {section.image && (
                <div className="relative group">
                  <img
                    src={section.image}
                    alt={section.title}
                    className="w-full rounded-2xl shadow-2xl object-cover max-h-[400px] transition-transform duration-300 group-hover:scale-[1.02]"
                  />
                </div>
              )}
            </div>
            ...
            <div className="space-y-8 mb-12">
              ...
              <div className="grid grid-cols-1 gap-8">
                {section.mainTopics.codeExamples.components.map((component, index) => (
                  <div key={index} className="bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-shadow duration-300">
                    <div className="p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {section.mainTopics.additionalResources.links.map((link, index) => (
                  <a
                    ...
                    className="flex items-center p-4 bg-blue-50 rounded-xl hover:bg-blue-100 transition-colors duration-300"
                  >
                    <span className="flex-shrink-0 bg-blue-100 rounded-full p-2 mr-4">
                      <svg className="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                      </svg>
                    </span>
                    <span className="text-gray-700 text-lg hover:text-blue-600 transition-colors duration-300">
                      {link.name}
                    </span>
                  </a>
                ))}
...
export default memo(ReactStateManagementOrta);