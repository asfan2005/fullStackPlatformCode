...
import practiceArguments from '../data/dataArguments';

function Arguments({ handleSubLessonComplete, activeLesson, activeSubLesson }) {
  const [currentExerciseIndex, setCurrentExerciseIndex] = useState(0);
  const [code, setCode] = useState(practiceArguments[0].initialCode);
  const [output, setOutput] = useState('');
  const [isHintModalOpen, setIsHintModalOpen] = useState(false);
  const [isCompleted, setIsCompleted] = useState(false);

  const currentExercise = practiceArguments[currentExerciseIndex];
  ...
  const runCode = () => {
    try {
      const sandbox = new Function('console', `
        ...
        ${code}
        // Test cases
        switch(${currentExerciseIndex + 1}) {
          case 1:
            consoleOutput.push(sumAll(1,2,3,4,5));
            break;
          case 2:
            consoleOutput.push(findMax(1,5,3,2,4));
            break;
          case 3:
            consoleOutput.push(checkArgs(5, "hello"));
            break;
          case 4:
            consoleOutput.push(calculate(5,3,'+'));
            break;
          case 5:
            consoleOutput.push(countArgs(1,2,3));
            break;
          case 6:
            consoleOutput.push(filterNumbers(1,"hello",2,"world",3));
            break;
          case 7:
            consoleOutput.push(concat(1,2,3,"hello"));
            break;
          case 8:
            consoleOutput.push(toObject("a",1,"b",2,"c",3));
            break;
          case 9:
            consoleOutput.push(validate(1,2,3));
            break;
          case 10:
            consoleOutput.push(square(1,2));
            break;
        }
      ...
      if (currentExercise.checkResult(consoleOutput)) {
        if (currentExerciseIndex < practiceArguments.length - 1) {
          setOutput(prevOutput => `${prevOutput}\n✅ To'g'ri! Ajoyib natija!`);
          setTimeout(() => {
            setCurrentExerciseIndex(prev => prev + 1);
            setCode(practiceArguments[currentExerciseIndex + 1].initialCode);
            setOutput('');
          }, 1500);
        } else {
          setIsCompleted(true);
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="w-full sm:w-auto flex flex-wrap gap-2">
          {currentExerciseIndex < practiceArguments.length - 1 && (
            <button
