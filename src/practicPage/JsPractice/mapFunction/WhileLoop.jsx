...
import practiceWhileLoop from '../data/dataWhileLoop';

function WhileLoop({ handleSubLessonComplete, activeLesson, activeSubLesson }) {
  const [currentExerciseIndex, setCurrentExerciseIndex] = useState(0);
  const [code, setCode] = useState(practiceWhileLoop[0].initialCode);
  const [output, setOutput] = useState('');
  const [isHintModalOpen, setIsHintModalOpen] = useState(false);
  const [isCompleted, setIsCompleted] = useState(false);

  const currentExercise = practiceWhileLoop[currentExerciseIndex];
  ...
  const runCode = () => {
    try {
      ...
      if (currentExercise.checkResult(consoleOutput)) {
        if (currentExerciseIndex < practiceWhileLoop.length - 1) {
          setOutput(prevOutput => `${prevOutput}\n✅ To'g'ri! Ajoyib natija!`);
          setTimeout(() => {
            setCurrentExerciseIndex(prev => prev + 1);
            setCode(practiceWhileLoop[currentExerciseIndex + 1].initialCode);
            setOutput('');
          }, 1500);
        } else {
          setIsCompleted(true);
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="w-full sm:w-auto flex flex-wrap gap-2">
          {currentExerciseIndex < practiceWhileLoop.length - 1 && (
            <button
