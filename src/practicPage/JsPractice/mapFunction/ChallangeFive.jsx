...
import practiceDataChallangeFive from '../data/dataChallangeFive';

function ChallangeFive({ handleSubLessonComplete, activeLesson, activeSubLesson }) {
  const [currentExerciseIndex, setCurrentExerciseIndex] = useState(0);
  const [code, setCode] = useState(practiceDataChallangeFive[0].initialCode);
  const [output, setOutput] = useState('');
  const [isHintModalOpen, setIsHintModalOpen] = useState(false);
  const [isCompleted, setIsCompleted] = useState(false);

  const currentExercise = practiceDataChallangeFive[currentExerciseIndex];

  useEffect(() => {
    const completedLessons = JSON.parse(localStorage.getItem('completedLessons') || '{}');
    if (completedLessons['2-5']) {
      setIsCompleted(true);
    }
  }, []);
  ...
  const runCode = () => {
    try {
      ...
      if (currentExercise.checkResult(consoleOutput)) {
        if (currentExerciseIndex < practiceDataChallangeFive.length - 1) {
          setOutput(prevOutput => `${prevOutput}\n✅ To'g'ri! Ajoyib natija!`);
          setTimeout(() => {
            setCurrentExerciseIndex(prev => prev + 1);
            setCode(practiceDataChallangeFive[currentExerciseIndex + 1].initialCode);
            setOutput('');
          }, 1500);
        } else {
          setIsCompleted(true);
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="w-full sm:w-auto flex flex-wrap gap-2">
          {currentExerciseIndex < practiceDataChallangeFive.length - 1 && (
            <button
